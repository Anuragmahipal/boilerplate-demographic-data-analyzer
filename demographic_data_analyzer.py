...
def calculate_demographic_data(print_data=True):
    ...
    # Fix column names (optional but helps)
    ...
    # Higher education mask
    higher_ed = df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])
    lower_ed = ~higher_ed

    # Percentage with salary >50K for both groups
    higher_education_rich = round((df[higher_ed]['salary'] == '>50K').mean() * 100, 1)
    lower_education_rich = round((df[lower_ed]['salary'] == '>50K').mean() * 100, 1)
